"""
This script will be used for training our CNN + RNN for scale target. The only input argument it
...
def main(config_file):
    ...
    training_generator = models.DataGenerator(
        train_df, config["model"]["batch_size"], output="scale"
    )
    validation_generator = models.DataGenerator(
        val_df, config["model"]["batch_size"], output="scale"
    )
