"""
This script will be used for training our CNN + RNN for both targets. The only input argument it
...
def main(config_file):
    ...
    training_generator = models.DataGenerator(
        train_df, config["model"]["batch_size"], output="both"
    )
    validation_generator = models.DataGenerator(
        val_df, config["model"]["batch_size"], output="both"
    )
    ...
    model = models.action_model_multi_output(
        INSHAPE,
        len(movement_names),
        len(scale_names),
