...
def get_class_names(config):
    ...
    """
    return sorted(
        os.listdir(os.path.join(config["data"]["directory"]))
    )
...
def predict_from_folder(folder, model, input_size, class_names):
    ...
    for dirpath, filename in walkdir(folder):
        ...
        img = keras.utils.load_img(path_image, target_size=input_size)
        x = keras.preprocessing.image.img_to_array(img)
        x = np.expand_dims(x, axis=0)

        # Get predictions
        preds = model.predict(x)
        index_pred = preds.argmax()
        prediction_decoded = class_names[index_pred]
